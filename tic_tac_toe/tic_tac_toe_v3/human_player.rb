class HumanPlayer
    ...
    def get_position(legal)
        ...
        input = nil
        input_arr = []
        until legal.include?(input_arr)
            puts "Player #{@mark.to_s} enter position as row and column as 2 numbers with a space in between  '1 1'"
            input = gets.chomp
            input_arr = input.split(" ")
            input_arr.map! {|ele| ele.to_i}
            
            puts "position is not a legal choice" if !legal.include?(input_arr)
            raise "invalid format" if input_arr.length != 2
        end
        input_arr
    end
